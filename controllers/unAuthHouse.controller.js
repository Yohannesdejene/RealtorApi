...
exports.search = async (req, res, next) => {
  ...
  try {
    ...
    const countarrayResults = Array.from(cousetSearchResult, (jsonString) =>
      JSON.parse(jsonString)
    );
    const newTotal = countarrayResults.length;

    // Convert Set to an array of JavaScript objects
    const arrayResults = Array.from(searchResults, (jsonString) =>
      JSON.parse(jsonString)
    );
    ...
    const filteredHouses = await House.findAll({
      where: {
        [Op.and]: [
          ...
          furnishingType && furnishingType !== "Any"
          ...
          minArea && maxArea && minArea !== "Any" && maxArea !== "Any"
            ? {
                areaSize: {
                  [Op.between]: [parseFloat(minArea), parseFloat(maxArea)],
                },
              }
            : minArea === "Any" && maxArea !== "Any"
            ? { areaSize: { [Op.lte]: parseFloat(maxArea) } }
            : minArea !== "Any" && maxArea === "Any"
            ? { areaSize: { [Op.gte]: parseFloat(minArea) } }
            : null,
...
exports.filteredHousesByRealestates = async (req, res, next) => {
  ...
  try {
    const filteredHouses = await House.findAll({
      where: {
        [Op.and]: [
          realestateType && realestateType !== "Any"
